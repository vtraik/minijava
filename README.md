...
- <u>ClassInfo</u>:  
Κάθε κλάση έχει ένα δύο δομές για τις μεθόδους, ένα LinkedHashMap που κάνει map (foo_int_boolean -> MethodInfo) και ένα (foo -> List<MethodInfo>).
...
- <u>MethodInfo</u>:  
Κρατούνται οι παράμετροι σε λίστα και οι τοπικές μεταβλητές σε HashMap. Επίσης υπάρχει ο τύπος επιστροφής μαζί με το όνομα σε 1 Symbol, το όνομα μαζί με τις παραμέτρους (foo_int_boolean), αν είναι μία override μέθοδος ή όχι και το offset της μεθόδου μέσα στη κλάση που βρίσκεται.
...
- <u>checkOverloadingSuperClass</u>:  
