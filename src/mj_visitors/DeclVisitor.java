...
class DeclVisitor extends GJDepthFirst<String, String> {
  ...
  private boolean checkTypes(List<Symbol> l1, List<Symbol> l2) throws Exception {
    for (int i = 0; i < l1.size(); ++i) {
      ...
      if (!(subtype(symbt, t1, t2) || subtype(symbt, t2, t1))) return true; // valid overload
    }
    return false;
  }
    ...
    for (int i = 0; i < methods.size(); ++i) {
      for (int j = i + 1; j < methods.size(); ++j) {
        ...
        if (!checkTypes(m1.getParams(), m2.getParams()))
          throw new Exception(
              String.format("Invalid overload -> %s.%s", className, m1.getRetId().getName()));
      }
    }
  }

  private void checkOverloadingSuperClass(List<MethodInfo> meth, String className, String methName)
      throws Exception {
      ...
      for (int i = 0; i < m1.size(); ++i) {
        ...
        for (int j = 0; j < m2.size(); ++j) {
          ...
          if (!checkTypes(m1.get(i).getParams(), m2.get(j).getParams()))
            throw new Exception(
                String.format(
                    "Invalid overload between %s.%s and %s.%s",
                    className, m1.get(i).getMangName(), currSuperName, m2.get(j).getMangName()));
        }
      }
    }
  }
  ...
  public String visit(Goal n, String argu) throws Exception {
    ...
    if (!undefinedTypes.isEmpty())
      throw new Exception(
          String.format("Class %s isn't defined", undefinedTypes.iterator().next()));
  ...
  public String visit(MainClass n, String argu) throws Exception {
    ...
    MethodInfo mainMeth =
        new MethodInfo(retId, "main_String[]", true); // dont count offset for main
  ...
  public String visit(ClassDeclaration n, String argu) throws Exception {
    ...
    undefinedTypes.remove(
        className); // remove the undefined type that is now being defined (if of course it is)
  ...
  public String visit(MethodDeclaration n, String argu) throws Exception {
    ...
    while ((superClass = symbt.getSuper(currSuperName)) != null) {
      ...
      if (superMethRetType != null && !methRetType.equals(superMethRetType)) {
        throw new Exception(
            String.format(
                "Override return types don't match -> class %s (%s-%s)",
