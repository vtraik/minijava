...
public class ClassInfo {
  ...
  public String getMethodRetType(String mangName) {
    return methodsSignatures.containsKey(mangName)
        ? methodsSignatures.get(mangName).getRetId().getType()
        : null;
  }
  ...
  public void addMethod(MethodInfo method) throws Exception {
    ...
    if (methodsSignatures.containsKey(method.getMangName()))
      throw new Exception(
          String.format("Method %s is already defined in this scope -> class %s", methName, name));
  ...
  public void addField(Symbol field) throws Exception {
    ...
    if (fields.containsKey(fieldName))
      throw new Exception(
          String.format("Symbol %s is already defined in this scope -> class %s", fieldName, name));

    field.setOffset(currFieldOffs);
    fields.put(fieldName, field);

    String type = field.getType();
    if (type.equals("int")) currFieldOffs += 4;
    else if (type.equals("boolean")) currFieldOffs += 1;
    else currFieldOffs += 8;
  }
    ...
    switch (choice) {
      ...
      case 1:
        ...
        for (Map.Entry<String, MethodInfo> meth : methodsSignatures.entrySet()) {
          int offs = meth.getValue().getOffset();
          boolean isOverridden = meth.getValue().getOverridden();
          if (!isOverridden) System.out.println(name + "." + meth.getKey() + " : " + offs);
        }
    }
  }
}
