...
public class Utils {
    ...
    public static boolean subtyperec(SymbolTable symbt, String type1, String type2) {
        ...
        if(superClass == null)
            ...
            return subtyperec(symbt, superClass.getName(), type2);
    }

    public static MethodInfo getClassCompMethod(SymbolTable symbt, List<MethodInfo> classMeths, String[] args) throws Exception {
        int argMatched = -1;
        for(int i = 0; i < classMeths.size(); ++i){
            ...
            for(int j = 0; j < numParams; ++j){
                ...
                if(!subtype(symbt, args[j], methType))
                    break;
                ++argMatched;
            }
